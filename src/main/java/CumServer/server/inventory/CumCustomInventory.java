package CumServer.server.inventory;
...
public class CumCustomInventory implements InventoryHolder {
    private final IInventory inventory;
    private final CraftInventory container;

    public CumCustomInventory(IInventory inventory) {
        this.container = new CraftInventory(inventory);
        this.inventory = inventory;
    }

    public CumCustomInventory(ItemStackHandler handler) {
        this.container = new CraftInventoryCustom(this, handler.getStacksList());
        this.inventory = this.container.getInventory();
    }

    public CumCustomInventory(InventoryPlayer playerInventory) {
    ...
    public static InventoryHolder getHolderFromForge(IItemHandler handler) {
        ...
        if (handler instanceof ItemStackHandler) return new CumCustomInventory((ItemStackHandler) handler);
        if (handler instanceof SlotItemHandler) return new CumCustomInventory(((SlotItemHandler) handler).inventory);
        if (handler instanceof InvWrapper) return new CumCustomInventory(((InvWrapper) handler).getInv());
        if (handler instanceof SidedInvWrapper) return new CumCustomInventory((IInventory) ReflectionHelper.getPrivateValue(SidedInvWrapper.class, (SidedInvWrapper) handler, "inv"));
        if (handler instanceof PlayerInvWrapper)  return new CumCustomInventory(getPlayerInv((PlayerInvWrapper) handler));
        return null;
    }
