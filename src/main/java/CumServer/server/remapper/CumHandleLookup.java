...
public class CumHandleLookup {
    private static HashMap<String, String> map = new HashMap<>();

    static {
        try {
            CumHandleLookup.loadMappings(new BufferedReader(new InputStreamReader(MappingLoader.class.getClassLoader().getResourceAsStream("mappings/" + CumServer.getNativeVersion() + "/cb2srg.srg"))));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
